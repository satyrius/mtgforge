...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'DataProviderPage.provider'
        db.add_column('oracle_dataproviderpage', 'provider',
                      self.gf('contrib.fields.NullCharField')(max_length=10, null=True, blank=True),
                      keep_default=False)

        # Adding field 'DataSource.provider'
        db.add_column('oracle_datasource', 'provider',
                      self.gf('contrib.fields.NullCharField')(max_length=10, null=True, blank=True),
                      keep_default=False)

        # Adding unique constraint on 'DataSource', fields ['content_type', 'object_id', 'provider']
        db.create_unique('oracle_datasource', ['content_type_id', 'object_id', 'provider'])


    def backwards(self, orm):
        # Removing unique constraint on 'DataSource', fields ['content_type', 'object_id', 'provider']
        db.delete_unique('oracle_datasource', ['content_type_id', 'object_id', 'provider'])

        # Deleting field 'DataProviderPage.provider'
        db.delete_column('oracle_dataproviderpage', 'provider')

        # Deleting field 'DataSource.provider'
        db.delete_column('oracle_datasource', 'provider')
